...
export class Universe {
    ...
    /**
     * Returns a copy of the cell buffer (JS: Uint8Array).
     * @returns {Uint8Array}
     */
    get_cells() {
    ...
    /**
     * Create a blank universe (all cells dead).
     * @param {number} width
     * @param {number} height
     * @returns {Universe}
     */
     ...
     * `density` is a float [0.0, 1.0]: probability a cell starts alive.
    ...
    /**
     * Set a specific cell (value = 0 or 1).
     * @param {number} row
     * @param {number} col
     * @param {number} value
     */
    ...
    /**
     * Reseed the internal RNG (call with `Date.now()` for variety).
     * @param {number} seed
     */
    set_seed(seed) {
        wasm.universe_set_seed(this.__wbg_ptr, seed);
    }
    /**
     * Advance one generation using standard B3/S23 rules on a toroidal grid.
     */
    tick() {
        wasm.universe_tick(this.__wbg_ptr);
    }
    /**
     * Advance `steps` generations in one call (useful for fast-forward).
     * @param {number} steps
     */
    tick_n(steps) {
        wasm.universe_tick_n(this.__wbg_ptr, steps);
    }
    /**
     * Flip a cell's state.
     * @param {number} row
...
async function __wbg_init(module_or_path) {
    ...
    if (module_or_path === undefined) {
        module_or_path = new URL('rust_life_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
